...
{
    public bool SHOW_COLLIDER = true; //!Remove before publish
    ...
    private void Update()
    {
        if (currentSpawnZ - cameraContainer.position.z < DISTANCE_BEFORE_SPAWN)
        {
            ...
            if (ammountOfActiveSegments >= MAX_SEGMENT_ON_SCREEN)
            {
    ...
    {
        List<Segment> possibleTransistion = availableTransitions.FindAll(x => x.beginY1 == y1 || x.beginY2 == y2 || x.beginY3 == y3);
        int id = Random.Range(0, possibleTransistion.Count);
